...
- Configuration (extend `src/core/config.py`):
  ...
  - `KORREL8R_MAX_ITEMS` (caps per alert and total)
  - `KORREL8R_TIME_SKEW_SEC` (± window around alert timestamp)
  - `KORREL8R_SEVERITY_THRESHOLD` (e.g., `critical`)
...
Proposed tools:
1) `korrel8r_find_related`
   - Purpose: Given a starting signal/object and a time window, return correlated results.
   - Input schema (JSON):
     - `start`: `{ "class": string, "labels": object, "timestamp?": string(ISO8601) }`
     - `targets?`: `string[]` (default from config `KORREL8R_TARGETS`)
     - `timeWindow?`: `{ "start": string(ISO8601), "end": string(ISO8601) }`
     - `limit?`: number
     - `depth?`: number
   - Output schema:
     - `{ "results": { "k8s/object"?: object[], "k8s/event"?: object[], "loki/log"?: object[], "tempo/trace"?: object[] }, "meta"?: object }`

2) `korrel8r_health`
   - Purpose: Probe liveness/readiness of Korrel8r and report available stores.
   - Input: none
   - Output: `{ "status": "ok"|"degraded"|"down", "stores"?: object }`

3) `korrel8r_build_links`
   - Purpose: Build deep links for correlated entities using configured bases.
   - Input schema:
     - `entities`: array of `{ "type": "k8s/object"|"loki/log"|"tempo/trace", ... }`
     - `window?`: `{ "start": string(ISO8601), "end": string(ISO8601) }`
   - Output: entities with `link` fields populated (Console / Grafana / Tempo), best-effort.

Behavior:
- All tools respect `KORREL8R_ENABLED`; return actionable errors when disabled.
- Timeouts and limits enforced per config.
- Errors never crash MCP process; return structured error messages.

Security:
- Use in-cluster auth as configured for REST client.
- Redact sensitive data in logs; avoid echoing raw log lines unless explicitly requested.

Examples:
```json
// korrel8r_find_related input
{
  "start": {"class": "prom/alert", "labels": {"alertname": "KubePodCrashLooping", "namespace": "llm-serving", "pod": "p-abc"}},
  "targets": ["k8s/object", "k8s/event", "loki/log"],
  "timeWindow": {"start": "2025-10-02T11:07:31Z", "end": "2025-10-02T11:27:31Z"},
  "limit": 50,
  "depth": 2
}

// korrel8r_find_related output
{
  "results": {
    "k8s/object": [{"kind": "Pod", "name": "p-abc", "namespace": "llm-serving"}],
    "k8s/event": [{"reason": "BackOff", "message": "Restarting"}],
    "loki/log": [{"ts": "2025-10-02T11:16:15Z", "line": "OOMKilled"}]
  },
  "meta": {"durationMs": 372}
}
```

### Response Contract Additions (Server → UI)
- Extend alerts responses to include:
  - `correlations`: structured array per alert with:
    - `resources`: list of `{kind, name, namespace, uid?, link?}`
    - `events`: list of concise Kubernetes events with timestamps
    - `logs`: `{query, sample_lines[], link}`
    - `traces`: `{trace_id, link}`
    - `confidence`: derived score based on rule distance/time proximity/label overlap
  - A brief, human-readable section appended to the alert analysis summarizing top correlated findings.
...
KORREL8R_MAX_ITEMS=50
KORREL8R_DEPTH=2

# Time window around alert timestamp
KORREL8R_TIME_SKEW_SEC=600

# Which alerts trigger correlation
KORREL8R_SEVERITY_THRESHOLD=critical

# Store/class selection (comma-separated)
KORREL8R_TARGETS=k8s/object,k8s/event,loki/log,tempo/trace
...
## Appendix H: MCP Tool Schemas (OpenAPI-like)

This appendix describes the MCP tool parameter/result schemas using an OpenAPI-like notation to aid validation and client integration. Tools are invoked via MCP, not HTTP, but the shapes mirror JSON payloads.

```yaml
openapi: 3.0.0
info:
  title: MCP Korrel8r Tools
  version: 0.1.0
  ...
  schemas:
    StartObject:
      ...
      properties:
        class:
          type: string
          description: Source class of the starting signal (e.g., prom/alert)
          example: prom/alert
        labels:
          type: object
          additionalProperties: true
          description: Label map used to identify the starting signal
          example:
            alertname: KubePodCrashLooping
            namespace: llm-serving
            pod: vllm-inference-7d6c9d6bfc-abc12
        timestamp:
          ...
          format: date-time
          nullable: true
          description: Optional anchor timestamp for correlation
      ...
      properties:
        results:
          ...
          properties:
            k8s/object:
              type: array
              items: { $ref: '#/components/schemas/K8sObject' }
            k8s/event:
              type: array
              items: { $ref: '#/components/schemas/K8sEvent' }
            loki/log:
              type: array
              items: { $ref: '#/components/schemas/LokiLog' }
            tempo/trace:
              type: array
              items: { $ref: '#/components/schemas/TempoTrace' }
        meta:
          type: object
          properties:
            durationMs: { type: integer }
            traversalDepth: { type: integer }
            errors: { type: array, items: { type: string } }

paths:
  mcp:korrel8r_find_related:
    post:
      summary: From a starting object, return correlated results
      requestBody:
        required: true
        content:
          application/json:
            schema:
              ...
              required: [start]
              properties:
                start: { $ref: '#/components/schemas/StartObject' }
                targets:
                  type: array
                  items: { type: string, enum: [k8s/object, k8s/event, loki/log, tempo/trace] }
                  description: Desired target classes; defaults from KORREL8R_TARGETS
      ...
      responses:
        '200':
          ...
          content:
            application/json:
              schema: { $ref: '#/components/schemas/Korrel8rResults' }
        '400': { description: Invalid input }
        '503': { description: Korrel8r disabled or unavailable }

  mcp:korrel8r_health:
    get:
      summary: Health and store availability
      responses:
        '200':
            ...
            application/json:
              schema:
                ...
                properties:
                  ...
                  stores: { type: object, additionalProperties: { type: string } }

  mcp:korrel8r_build_links:
    post:
      summary: Populate deep links for correlated entities
      requestBody:
        required: true
        content:
          application/json:
                ...
                entities:
                  ...
                  items:
                    oneOf:
                      ...
                      - $ref: '#/components/schemas/LokiLog'
                      - $ref: '#/components/schemas/TempoTrace'
                window: { $ref: '#/components/schemas/TimeWindow' }
      responses:
        '200':
          description: Entities with links populated where possible
          content:
            application/json:
              schema:
                type: object

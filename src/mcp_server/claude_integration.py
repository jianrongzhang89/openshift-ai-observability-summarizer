...
class PrometheusChatBot:
    ...
    def _convert_mcp_tools_to_claude_format(self) -> List[Dict[str, Any]]:
        ...
        claude_tools = [
            ...
            },
            {
                ...
                "description": "Execute a Korrel8r domain query (GET /api/v1alpha1/objects) to fetch raw objects via Korrel8r.",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "query": {
                            "type": "string",
                            "description": 'Korrel8r query string domain:class:selector. Examples: alert:alert:{"alertname":"PodDisruptionBudgetAtLimit"}, k8s:Pod:{"namespace":"llm-serving"}, loki:log:{"kubernetes.namespace_name":"llm-serving","kubernetes.pod_name":"p-abc"}, trace:span:{".k8s.namespace.name":"llm-serving"}'
                        }
                    },
                    "required": ["query"]
                }
    ...
    def _route_tool_call_to_mcp(self, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if tool_name == "korrel8r_query_objects":
                try:
